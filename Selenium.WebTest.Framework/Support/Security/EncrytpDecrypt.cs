...
namespace Selenium.WebTest.Framework.Support.Security
{
    //Using AES Rijndael algorithm
    public class EncrytpDecrypt
    {
        ...
        public static byte[] Encrypt(byte[] clearData, byte[] Key, byte[] IV)
        {
            ...
            // The IV is XORed with the first block (8 byte)  of the data before it is encrypted, 
            // and then each encrypted block is XORed with the following block of plaintext.
            ...
            // CryptoStreamMode.Write means that we are going to bewriting data to the stream and the output will be written in the MemoryStream we have provided. 
        ...
        public static string Encrypt(string clearText, string password)
        {
            ...
            // You can also read KeySize/BlockSize properties offthe algorithm to find out the sizes. 
        ...
        public static byte[] Decrypt(byte[] cipherData, byte[] Key, byte[] IV)
        {
            ...
            // Create a symmetric algorithm. 
            // We are going to use Rijndael because it is strong and available on all platforms. 
            // You can use other algorithms, to do so substitute the nextline with something like 
            //     TripleDES alg = TripleDES.Create(); 
            Rijndael alg = Rijndael.Create();

            // Now set the key and the IV. 
            // We need the IV (Initialization Vector) because the algorithm is operating in its default mode called CBC (Cipher Block Chaining). 
            // The IV is XORed with the first block (8 byte) of the data after it is decrypted, 
            // and then each decrypted  block is XORed with the previous  cipher block. This is done to make encryption more secure. 
            // There is also a mode called ECB which does not need an IV, but it is much less secure. 
            alg.Key = Key;
            alg.IV = IV;
